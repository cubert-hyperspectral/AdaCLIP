"""CIR false-color AdaCLIP example using the cuvis_ai_adaclip plugin.

This script is intended to be run inside an environment where both
``cuvis_ai`` and ``cuvis_ai_adaclip`` are installed.

Pipeline:
    LentilsAnomalyDataNode → CIRFalseColorSelector → AdaCLIPDetector
    → QuantileBinaryDecider → AnomalyDetectionMetrics + visualizations.
"""
...
# From: AdaCLIP-cuvis/cuvis_ai_adaclip/examples_cuvis/statistical_cir_false_color.py
...
DEFAULT_EXPERIMENT_NAME = "statistical_adaclip_cir_false_color_plugin"
DEFAULT_MONITOR_ROOT = CUVIS_AI_ROOT / "outputs" / "tensorboard"
DEFAULT_QUANTILE = 0.995
DEFAULT_MASK_CHANNEL = 0
# CIR false-color defaults: NIR->R, R->G, G->B
DEFAULT_NIR_NM = 860.0
DEFAULT_RED_NM = 670.0
...
@hydra.main(version_base=None, config_path="../../../cuvis.ai/cuvis_ai/conf", config_name="general")
def main(cfg: DictConfig) -> None:
    ...
    # CIR false-color wavelengths
    nir_nm = float(getattr(cfg, "cir_nir_nm", DEFAULT_NIR_NM))
    red_nm = float(getattr(cfg, "cir_red_nm", DEFAULT_RED_NM))
    green_nm = float(getattr(cfg, "cir_green_nm", DEFAULT_GREEN_NM))
    ...
    logger.info("=== AdaCLIP CIR false-color example (plugin) ===")
    ...
    trainer = StatisticalTrainer(canvas=canvas, datamodule=datamodule)
