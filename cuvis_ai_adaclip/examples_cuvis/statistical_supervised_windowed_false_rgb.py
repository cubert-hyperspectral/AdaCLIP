"""Supervised windowed false-RGB AdaCLIP example (visible mRMR band selection).

This script mirrors the updated AdaCLIP examples and uses the shared experiment
config in `configs/experiment/adaclip_supervised_windowed_false_rgb.yaml`.

It:
  * Builds a CuvisPipeline explicitly.
  * Uses LentilsAnomalyDataNode → SupervisedWindowedFalseRGBSelector → AdaCLIPDetector.
...
from cuvis_ai.node.band_selection import SupervisedWindowedFalseRGBSelector
...
cli = AdaCLIPCLI("AdaCLIP Supervised Windowed False RGB")
...
def main(**kwargs):
    ...
    # Supervised windowed false-RGB band selection from CLI options
    windows = cli.parse_hc_windows(kwargs["sup_wf_windows"])
    score_weights = cli.parse_sup_score_weights(kwargs["sup_wf_score_weights"])
    lambda_penalty = kwargs["sup_wf_lambda_penalty"]
    ...
    logger.info("Supervised windowed windows: {}", windows)
    ...
    # Build pipeline
    # ----------------------------
    pipeline = CuvisPipeline("AdaCLIP_Supervised_WindowedFalseRGB")
    ...
    pipeline.connect(
        # hyperspectral → supervised windowed RGB
    ...
    if getattr(band_selector, "requires_initial_fit", False):
        logger.info("Running statistical fit for supervised windowed false-RGB band selector...")
        trainer.fit()
    ...
    pipeline_metadata = PipelineMetadata(
        name=pipeline.name,
        description=(
            "Statistical AdaCLIP supervised windowed false-RGB pipeline "
            "(LentilsAnomalyDataNode → SupervisedWindowedFalseRGBSelector → AdaCLIPDetector)"
        ),
        tags=["statistical", "adaclip", "supervised_windowed_false_rgb"],
        author="cuvis.ai",
    )
    ...
    pipeline_config_path = pipeline_config_dir / "adaclip_supervised_windowed_false_rgb.yaml"
    ...
    trainrun_config = TrainRunConfig(
        name="adaclip_supervised_windowed_false_rgb_cli",
    ...
    trainrun_output_path = results_dir / "adaclip_supervised_windowed_false_rgb_cli_trainrun.yaml"
